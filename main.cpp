...
private:
    ...
    QPushButton *createButton, *computeButton, *calcButton, *clearButton;
...
private slots:
    ...
    void computeDividedDifferences() {
        ...
        for (int i = 0; i < n; ++i) {
            x[i] = tableWidget->item(i, 0) ? tableWidget->item(i, 0)->text().toDouble() : 0.0;
            f[0][i] = tableWidget->item(i, 1) ? tableWidget->item(i, 1)->text().toDouble() : 0.0;
        }
...
int main(int argc, char *argv[])
{
    ...
    w.setWindowTitle("Divided Differences Interpolation");
    w.resize(800, 600);
    w.show();
...
private:
    QTableWidget *tableWidget;
    QPushButton *createButton, *computeButton, *calcButton, *clearButton;
    QTextEdit *equationDisplay;  // QTextEdit to display the equation
    ...
    void setupUI() {
        ...
        equationDisplay = new QTextEdit(this);  // Initialize QTextEdit for equation display
        equationDisplay->setReadOnly(true);  // Make it read-only
        equationDisplay->setStyleSheet("background-color: #f0f0f0; padding: 8px; border-radius: 8px;");
        ...
        layout->addWidget(tableWidget);
        layout->addWidget(equationDisplay);  // Place equation display above buttons
        layout->addWidget(createButton);
        layout->addWidget(computeButton);
        layout->addWidget(calcButton);
        layout->addWidget(clearButton);
...
private slots:
    ...
    void computeDividedDifferences() {
        ...
        // Simplified equation generation for Newton's method
        QString equation = "f(x) = " + QString::number(f[0][0]);
        for (int i = 1; i < n; ++i) {
            QString term = QString::number(f[i][0]);
            for (int j = 0; j < i; ++j) {
                term += " * (x - " + QString::number(x[j]) + ")";
            }
            equation += " + " + term;
        }

        equationDisplay->setText(equation);  // Display the equation in the QTextEdit
    }
    ...
    void clearTable() {
        ...
        if (reply == QMessageBox::Yes) {
            tableWidget->clearContents();
            equationDisplay->clear();  // Clear the equation display as well
        }
    }
};
...
#include <muParser.h>
#include <cmath>
#include <QTextEdit>  // إضافة هذا السطر لتضمين QTextEdit
...
private:
    ...
    void setupUI() {
        ...
        calcButton = new QPushButton("Calculate Value at X", this);
        clearButton = new QPushButton("Clear Table", this);
        equationDisplay = new QTextEdit(this);  // هنا
        ...
        layout->addWidget(equationDisplay);
        layout->addWidget(tableWidget);
        layout->addWidget(createButton);
        layout->addWidget(computeButton);
        layout->addWidget(calcButton);
...
private slots:
    ...
    void computeDividedDifferences() {
        ...
        QString equation = "f(x) = ";
        for (int i = 0; i < n; ++i) {
            equation += QString::number(f[i][0]) + (i < n - 1 ? " * (x - " + QString::number(x[i]) + ") + " : "");
        }
...
private slots:
    void createTable() {
        ...
        for (int i = 1; i <= n - 1; ++i) {
            if (i == 1) {
                computedEquation += QString::number(f[0][0]);
            }
            if (i > 1) {
                computedEquation += " + (" + QString::number(f[i - 1][0]) + ")";
                for (int k = 0; k < i - 1; ++k) {
                    computedEquation += " * (x - " + QString::number(x[k]) + ")";
                }
            }
        }
        headers << QString::number(i) + " Diff";
...
void computeDividedDifferences() {
    equationDisplay->clear();
    equationDisplay->setText(equation);
    QString equation = "P(x) = ";
    computedEquation.clear();
    int n = tableWidget->rowCount();
    if (n == 0) return;
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j < n - i; ++j) {
            f[i][j] = (f[i-1][j+1] - f[i-1][j]) / (x[j+i] - x[j]);
            if (j == 0 && i == 1) {
                equation += QString::number(f[i][j]);
            } else if (j == 0) {
                equation += " + (" + QString::number(f[i][j]) + ")";
                for (int k = 0; k < i; ++k) {
                    equation += " * (x - " + QString::number(x[k]) + ")";
                }
            }
            tableWidget->setItem(j, i+1, new QTableWidgetItem(QString::number(f[i][j])));
        }
...
void calculateValueAtX() {
    if (!equationDisplay->toPlainText().isEmpty()) {
        QMessageBox::information(this, "Equation", "Computed Equation: " + equationDisplay->toPlainText());
    }
    if (computedEquation.isEmpty()) {
        QMessageBox::warning(this, "Error", "Please compute the divided differences first.");
        return;
    }

    QMessageBox::information(this, "Computed Equation", "The computed equation is: " + computedEquation);
    int n = tableWidget->rowCount();
    if (n == 0) return;
